...
def convert(params):
    ...
    return 'python {path}pipeline/02_train_class_trees.py -i {path}data/{dataset_name}/02_bundled_train/ -v {path}data/{dataset_name}/02_bundled_test/ -o ../data/1000_dataset_1000000/03_tree/ -j 2 -c 5 -n 50 -b 1 -S {key1} -rs {key2} -ms {key3} -rt {key4} -mt {key5} -g 0 -d {key6} -s {key7} -t {key8} -umeanx {key9} -umeany {key10} -uvarx {key11} -uvary {key12} -uvarxy {key13} -vmeanx {key14} -vmeany {key15} -vvarx {key16} -vvary {key17} --vvarxy {key18}'.format(**param_dict)
...
def call(x, path=None, arrayid=None):
    ...
    try:
        ...
        cmd = '{} --max_node_count 1000000'.format(cmd)
        result = float(subprocess.check_output(cmd.split()))
    if not path is None:
